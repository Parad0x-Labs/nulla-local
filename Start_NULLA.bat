...
set "RECEIPT_HOME_FILE=%TEMP%\nulla_receipt_home_%RECEIPT_TAG%.txt"
set "RECEIPT_PROFILE_FILE=%TEMP%\nulla_receipt_profile_%RECEIPT_TAG%.txt"
set "RECEIPT_MODEL_FILE=%TEMP%\nulla_receipt_model_%RECEIPT_TAG%.txt"
if "%NULLA_HOME%"=="" (
...
"%PYTHON_EXE%" -m ops.ensure_public_hive_auth --project-root "%SCRIPT_ROOT%" --watch-host "%NULLA_PUBLIC_HIVE_WATCH_HOST%" >"%TEMP%\nulla_public_hive_auth.log" 2>&1
...
echo.
"%PYTHON_EXE%" -m apps.nulla_api_server
endlocal
